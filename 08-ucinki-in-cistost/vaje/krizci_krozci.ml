...
let prazna_vrstica : vrstica =  (None, None, None)

let empty_grid : grid = (prazna_vrstica, prazna_vrstica, prazna_vrstica)
...
let show_state = function
  ...
  | GameOver {result; final_grid} ->
      let winner_message =
        match result with
          | Tie -> "Tie"
          | Winner player -> "Zmagal je " ^show_player player ^ "!" in
